...
import { GitHubStore } from './github-store.js?v=20260916.1';
...
console.log('[MM] app.js v20260916.1 loaded');
...
class App {
  ...
  _openBhajanModal(id, context = null) {
    ...
    if (context && context.bhajans.length > 1) {
      prevBtn.classList.remove('invis');
  ...
  async _loadDashOpenSessions() {
    ...
    // Build a map of roomCode → {isHost, isDraft} for sessions known locally
  ...
  _renderLiveSession(el) {
    ...
    const displayBhajans = st.bhajans || [];
  ...
  async _openNewSession(series = null, backdated = false) {
    ...
    // Populate series dropdown; pre-select the series this was opened from
    ...
    const dateInput = document.getElementById('sf-date');
  ...
  _startPlaying() {
    ...
    // Append Mangala Aarati as the final navigable entry for play mode (stripped before save in _endSession/_exitPlay)
  ...
  _endSession() {
    ...
    const bhajans = (this.liveState.bhajans || []).filter(e => e.bhajan_id !== 'mangala-aarati');
    const singers = [...new Set(bhajans.flatMap(e => e.singers || (e.singer ? [e.singer] : [])))];
